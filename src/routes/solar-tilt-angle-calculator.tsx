...
export const Route = createFileRoute('/solar-tilt-angle-calculator')({
  component: TiltCalculator,
});

function TiltCalculator() {
  const calc = getCalculatorBySlug('/solar-tilt-angle-calculator');
  ...
  return (
    <CalculatorLayout 
      title={calc?.seoTitle || "Solar Tilt Angle Calculator"}
      description={calc?.seoDescription || "Estimate an appropriate solar panel tilt angle based on your location."}
      currentId="solar-tilt-angle-calculator"
      faq={calc?.faq}
      results={results ? <AdvancedResults results={results} type="tilt" /> : null}
      calculator={
        <SharedCalculator 
          calculatorId="solar-tilt-angle-calculator"
          ...
          initialInputs={{
            ...
            latitude: 34.05 // Default to something like LA if no location
          }}
        />
      }
    />
