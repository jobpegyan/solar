...
export const Route = createFileRoute("/solar-calculator/usa/$state")({
  component: StateSolarCalculator,
  head: ({ params }) => {
    ...
    return {
      ...
      meta: [
        { name: "description", content: `Estimate solar panel requirements, system costs, and electricity bill savings for ${stateName} homeowners. Accurate local utility rates and sun data.` },
      ],
    };
  },
});

function StateSolarCalculator() {
  ...
  const stateData = regions.find(
    r => r.countryCode === "US" && r.name.toLowerCase() === state.toLowerCase()
  );
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      ...
      <section className="mb-12">
        ...
        <p className="text-xl text-muted-foreground leading-relaxed max-w-3xl">
          Get a precise estimate for your solar installation in {stateName}. We account for local electricity rates, {stateData?.defaultPeakSunHours || "regional"} peak sun hours, and US-specific incentives.
        </p>
      </section>
      ...
      <section className="prose prose-slate max-w-none border-t pt-12">
        <h2>How Solar Works in {stateName}</h2>
        <p>
          Homeowners in {stateName} can significantly reduce their utility expenses by leveraging the state's {stateData?.defaultPeakSunHours && stateData.defaultPeakSunHours > 5 ? "abundant sunshine" : "solar potential"}. Sizing a system correctly requires balancing your annual electricity consumption with the local solar irradiance.
        </p>
        <h3>Calculation Factors for {stateName}:</h3>
        <ul>
          ...
          <li><strong>System Losses:</strong> 20-25% standard loss factor included for heat and equipment inefficiency.</li>
        </ul>
      </section>
